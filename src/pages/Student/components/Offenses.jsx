...
import { useStudentOffenses } from "../../../hooks/useOffenses";

const Offenses = () => {
  const { studentNumber } = useGlobalContext();
  const { data: offensesList = [], isLoading } = useStudentOffenses(studentNumber);
            ...
            {isLoading ? (
              ...
              offensesList.map((offense, index) => (
                <tr key={index} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-2">{new Date(offense.timestamp).toLocaleDateString()}</td>
